    ...
    where TValue : unmanaged
{
    ...
    */
    ...
    DictionarySlot<TKey, TValue>* _slots
    {
    ...
    public ValueDictionary(MemoryPoolCluster* memoryPool) : this(8, false, memoryPool) { }
    ...
    /// </summary>
    /// <param name="memoryPool"></param>
    public void Init(MemoryPoolCluster* memoryPool)
    {
        Init(8,false,memoryPool);
    }
    ...
    private int GetKeyHashCode(TKey* key)
    {
        ...
        if (typeof(TKey) == typeof(UnManagedMemory<char>) || typeof(TKey) == typeof(UnManagedCollection<char>))
        {
            return (int)((UnManagedMemory<char>*)key)->AsSpan().HashCode();
        }
    ...
    private bool AllocateMemory(uint capacity, out byte* ctrl)
    {
        ...
        if(!_onMemoryPool && basePtr is null)
            basePtr = (byte*)NativeMemory.AllocZeroed((nuint)totalSize);
    ...
    byte* Alloc(ulong size)
    {
        ...
        ALLOC_ON_HEAP:
        return (byte*)NativeMemory.AllocZeroed((nuint)size, SolamirareEnvironment.ALIGNMENT);
