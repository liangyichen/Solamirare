...
public static unsafe class UnManagedCollectionExtension
{
    ...
    public static UnManagedMemory<UnManagedString> Split(this in UnManagedCollection<char> source, char separator)
    {
        UnManagedMemory<UnManagedString> result = new UnManagedMemory<UnManagedString>();

        UnManagedStringHelper.Split(source.AsSpan(),separator,&result,false);
    ...
    public static ValueDictionary<UnManagedString, UnManagedString> SplitMapToValueDictionary(this ReadOnlySpan<char> source, char outerSymbol, char innerSymbol)
    {
        ValueDictionary<UnManagedString, UnManagedString> result;

            result = UnManagedStringHelper.SplitMapToValueDictionary(source, outerSymbol, innerSymbol);
    ...
    /// <summary>
    /// 把字符串分割为 ValueDictionary（复制到新的内存），使用两个符号做二维分割
    /// <para>Splits the string into a ValueDictionary (copied to new memory), using two symbols for two-dimensional splitting.</para>
    /// </summary>
    ...
    public static ValueDictionary<UnManagedString, UnManagedString> SplitCopyToValueDictionary(this ReadOnlySpan<char> source, char outerSymbol, char innerSymbol)
    {
        ValueDictionary<UnManagedString, UnManagedString> result

             = UnManagedStringHelper.SplitCopyToValueDictionary(source, outerSymbol, innerSymbol);
