...
public unsafe ref struct IoUringServer
{
    ...
    bool ProcessRequestWithContext(int file_descriptor)
    {
        ...
        try
        {
            //与IOCP版本一致，需要跨方法传递buffer，不可以使用 stackalloc 分配
            byte* response_buffer = ResponseBufferFor(file_descriptor);

            bool processed;

            processed = ServerFunctions.ProcessUserLogic(ServerConfig, user_logic_callback, context, response_buffer, &file_descriptor);
