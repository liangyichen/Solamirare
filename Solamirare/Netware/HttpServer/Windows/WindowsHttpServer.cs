...
internal unsafe ref struct WindowsHttpServer
{
    ...
    void OnRecv(byte* iocp_context_pointer, nint socket_handle, uint transferred_bytes)
    {
        ...
        //方法内后续没有直接输出的逻辑，输出buffer必须跨方法传递，不能使用stackalloc临时分配
        byte* response_buffer = response_buffer_pool_pointer + (nuint)slot_index * (nuint)ServerConfig->RESPONSE_BUFFER_CAPACITY;
         
        bool proceed = ServerFunctions.ProcessUserLogic(ServerConfig, user_logic_callback, context, response_buffer, &socket_handle);
