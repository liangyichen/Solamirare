...
public unsafe partial struct UHttpResponse
{
    ...
    private void EnsureHeaderSerialized()
    {
        ...
        *cur++ = 10; // \r\n
        ...
        // 上一个头部已经写入行尾 CRLF，这里再写一个 CRLF 结束头部。
        // The preceding header already wrote its trailing CRLF; add one more to terminate headers.
        *cur++ = 13; *cur++ = 10;
    ...
    private static bool serializeHeaderToBuffer(int index, UnManagedMemory<byte>* key, UnManagedMemory<byte>* value, void* caller)
    {
        ...
        if (*cur + needed > end)
        {
            // 释放当前 Header 的资源后 return true，让迭代继续处理剩余 Header。
            key->Dispose();
            value->Dispose();
            return true;
        }
