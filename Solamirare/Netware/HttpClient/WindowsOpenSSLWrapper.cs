...
using System.IO;
/// <summary>
/// 提供 Windows 平台下的 OpenSSL 动态绑定封装。
/// </summary>
public static unsafe class WindowsOpenSSLWrapper
{
    ...
    static WindowsOpenSSLWrapper()
    {
        // It's more robust to load libcrypto first, as libssl depends on it.
        // Removed libeay32.dll/ssleay32.dll (OpenSSL 1.0.x) as they don't support TLS_client_method.
        var cryptoNames = new List<string> { "libcrypto-3.dll", "libcrypto-1_1.dll", "libcrypto.dll" };
        var sslNames = new List<string> { "libssl-3.dll", "libssl-1_1.dll", "libssl.dll" };

        // For different architectures, some distributions use architecture-specific names. Add them to the search path.
        if (RuntimeInformation.ProcessArchitecture == Architecture.Arm64)
        {
            cryptoNames.Insert(0, "libcrypto-3-arm64.dll");
            cryptoNames.Insert(1, "libcrypto-1_1-arm64.dll");
            sslNames.Insert(0, "libssl-3-arm64.dll");
            sslNames.Insert(1, "libssl-1_1-arm64.dll");
        }
        else if (RuntimeInformation.ProcessArchitecture == Architecture.X64)
        {
            cryptoNames.Insert(0, "libcrypto-3-x64.dll");
            cryptoNames.Insert(1, "libcrypto-1_1-x64.dll");
            sslNames.Insert(0, "libssl-3-x64.dll");
            sslNames.Insert(1, "libssl-1_1-x64.dll");
        }

        // Define search paths
        var searchPaths = new List<string> { null }; // null = default system search (PATH, System32, etc.)
        try
        {
            string programFiles = Environment.GetFolderPath(Environment.SpecialFolder.ProgramFiles);
            searchPaths.Add(Path.Combine(programFiles, "OpenSSL-Win64", "bin"));
            searchPaths.Add(Path.Combine(programFiles, "OpenSSL", "bin"));
        }
        catch { }

        IntPtr cryptoHandle = IntPtr.Zero;
        string loadedPath = null;

        // Try to load libcrypto
        foreach (var path in searchPaths)
        {
            foreach (var name in cryptoNames)
            {
                string target = path == null ? name : Path.Combine(path, name);
                if (path != null && !File.Exists(target)) continue;

                if (NativeLibrary.TryLoad(target, out cryptoHandle))
                {
                    loadedPath = path;
                    break;
                }
            }
            if (cryptoHandle != IntPtr.Zero) break;
        }

        // If we found crypto in a specific path, prioritize that path for ssl
        if (loadedPath != null && loadedPath != searchPaths[0])
        {
            searchPaths.Insert(0, loadedPath);
        }

        // Now, load libssl.
        foreach (var path in searchPaths)
        {
            foreach (var name in sslNames)
            {
                string target = path == null ? name : Path.Combine(path, name);
                if (path != null && !File.Exists(target)) continue;

                if (NativeLibrary.TryLoad(target, out libHandle)) break;
            }
            if (libHandle != IntPtr.Zero) break;
        }

        // If either lib fails to load, HTTPS is not supported.
        if (libHandle == IntPtr.Zero || cryptoHandle == IntPtr.Zero)
        {
            SupportsHttps = false;
            return;
        }
