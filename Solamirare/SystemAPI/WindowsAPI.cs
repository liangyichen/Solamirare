...
internal unsafe partial class WindowsAPI
{
    ...
    /// <summary>
    /// 将 UTF-16 字符写入 Windows 控制台屏幕缓冲区。
    /// </summary>
    /// <param name="hConsoleOutput">控制台输出缓冲区句柄。</param>
    /// <param name="lpBuffer">待写入的 UTF-16 字符缓冲区。</param>
    /// <param name="nNumberOfCharsToWrite">要写入的字符数量。</param>
    /// <param name="lpNumberOfCharsWritten">接收实际写入的字符数量。</param>
    /// <param name="lpReserved">保留参数，必须为 null。</param>
    /// <returns>成功返回 true，否则返回 false。</returns>
    [LibraryImport("kernel32.dll", SetLastError = true)]
    [return: MarshalAs(UnmanagedType.Bool)]
    public static partial bool WriteConsoleW(void* hConsoleOutput, char* lpBuffer, uint nNumberOfCharsToWrite, out uint lpNumberOfCharsWritten, void* lpReserved);



    /// <summary>
    /// 获取进程的标准输入、标准输出或标准错误句柄。
    /// </summary>
    /// <param name="nStdHandle">标准句柄标识，例如 -10 表示标准输入，-11 表示标准输出。</param>
    /// <returns>成功返回句柄，失败返回 null 或 INVALID_HANDLE_VALUE。</returns>
    [LibraryImport("kernel32.dll", SetLastError = true)]
    public static partial void* GetStdHandle(int nStdHandle);



    /// <summary>
    /// 从 Windows 控制台输入缓冲区读取 UTF-16 字符。
    /// </summary>
    /// <param name="hConsoleInput">控制台输入缓冲区句柄。</param>
    /// <param name="lpBuffer">接收字符的缓冲区。</param>
    /// <param name="nNumberOfCharsToRead">最多读取的字符数量。</param>
    /// <param name="lpNumberOfCharsRead">接收实际读取的字符数量。</param>
    /// <param name="pInputControl">保留参数，通常为 null。</param>
    /// <returns>成功返回 true，否则返回 false。</returns>
    [LibraryImport("kernel32.dll", SetLastError = true)]
    [return: MarshalAs(UnmanagedType.Bool)]
    public static partial bool ReadConsoleW(void* hConsoleInput, char* lpBuffer, uint nNumberOfCharsToRead, out uint lpNumberOfCharsRead, void* pInputControl);
    ...
    // /// <summary>
    // /// 从文件、管道或控制台句柄同步读取字节数据。
    // /// </summary>
    // /// <param name="hFile">文件、管道或输入设备句柄。</param>
    // /// <param name="lpBuffer">接收字节的缓冲区。</param>
    // /// <param name="nNumberOfBytesToRead">最多读取的字节数量。</param>
    // /// <param name="lpNumberOfBytesRead">接收实际读取的字节数量。</param>
    // /// <param name="lpOverlapped">同步读取时必须为 null。</param>
    // /// <returns>成功返回 true，否则返回 false。</returns>
    // [LibraryImport("kernel32.dll", SetLastError = true)]
    // [return: MarshalAs(UnmanagedType.Bool)]
    // public static unsafe partial bool ReadFile(void* hFile, byte* lpBuffer, uint nNumberOfBytesToRead, out uint lpNumberOfBytesRead, void* lpOverlapped);
