...
namespace Solamirare
{
    ...
    public record Photon
    {
        ...
        /// <summary>
        /// Aot不具备动态判断能力，不能用 dynamic 作为值， 所以这里暂时先用 string
        /// </summary>
        ...
        public IEnumerable<KeyValuePair<string, string>> Export(params string[] keys)
        {
            ...
            {
                var enm = Data.GetEnumerator();

                while (enm.MoveNext())
                {

                    Span<string> spanKeys = keys.AsSpan();

                    if (spanKeys.IndexOf(enm.Current.Key) > -1)
                    {
        ...
        /// <param name="append"></param>
        /// <returns></returns>
        public Photon Fusion(Photon append)
        {
            if(append is not null)
            Import(append.Data);
        ...
        public Photon Set(string name, string value)
        {
            ...
            if (!string.IsNullOrWhiteSpace(name))
            {
                ...
                if (Data.ContainsKey(name))
                {
                    ...
                    if (!success && retryCount == 1)  //遇到多线程同时写问题，重试 1 次
                        goto tryUpdate;
