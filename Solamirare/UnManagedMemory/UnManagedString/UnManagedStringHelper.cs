...
public static unsafe partial class UnManagedStringHelper
{
    ...
    public static UnManagedMemory<UnManagedMemory<char>> SplitMapToCollection(ReadOnlySpan<char> source, char separator)
    {
        if (source.IsEmpty)
            return UnManagedMemory<UnManagedMemory<char>>.Empty;
 

       
        UnManagedMemory<UnManagedMemory<char>> result = new();

        Split(source, separator, &result, false);
    ...
    public static ValueDictionary<UnManagedString, UnManagedString> SplitCopyToValueDictionary(ReadOnlySpan<char> source, char entrySeparator, char keyValueSeparator)
    {
        ...
        UnManagedMemory<UnManagedMemory<char>> nodes = SplitMapToCollection(source, entrySeparator); //得到第一个分隔符分离的字段

        ValueDictionary<UnManagedString, UnManagedString> result = new(nodes.UsageSize);

        for (int i = 0; i < nodes.UsageSize; i++)
        {
            UnManagedMemory<char>* segment = nodes[i];
            ...
            if (!keySpan.IsEmpty)
            {
                ...
                bool ok = result.Add(&keyMem, &valueMem);

                if (!ok)
                {
                    keyMem.Dispose();
                    if (!valueSpan.IsEmpty) valueMem.Dispose();
                }
    ...
    /// <returns>包含键值对的字典。</returns>
    public static ValueDictionary<UnManagedString, UnManagedString> SplitMapToValueDictionary(ReadOnlySpan<char> source, char entrySeparator, char keyValueSeparator)
    {
        if (source.IsEmpty)
            return ValueDictionary<UnManagedString, UnManagedString>.Empty;

        UnManagedMemory<UnManagedMemory<char>> nodes = SplitMapToCollection(source, entrySeparator);
    ...
    /// <returns></returns>
    public static UnManagedMemory<UnManagedString> Split(this in UnManagedString source, char separator, bool clone)
    {
        UnManagedMemory<UnManagedString> result = new UnManagedMemory<UnManagedString>();

        Split(source.AsSpan(),separator,&result,clone);
    ...
    public static bool Split(ReadOnlySpan<char> source, char separator, UnManagedMemory<UnManagedString>* result, bool clone)
    {

        if (source.IsEmpty || result is null || !result->Activated) return false;
        ...
        if (resize_result)
        {
            while (true)
            {
                int separatorIndex = source.Slice(start).IndexOf(separator);

                if (separatorIndex == -1)
                {
                    index++;

                    UnManagedString endof = source.Slice(start);

                    if(clone)
                    {
                        result->Add(endof.Clone());
                    }
                    else
                    {
                        result->Add(endof);
                ...
                else
                {
                    ...
                    UnManagedString node = source.Slice(start, separatorIndex);
