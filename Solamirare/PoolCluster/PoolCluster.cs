...
public unsafe struct MemoryPoolCluster
{
    ...
    //分配内存时是否加锁
    private bool allocLock;

    //归还内存时是否清空内容
    private bool clearWhenReturn;
    ...
    /// <param name="allocLock">分配内存时是否加锁</param>
    /// <param name="clearWhenReturn">归还内存时是否清空内容</param>
    /// <returns></returns>
    public bool Init(Span<MemoryPoolSchema> schemas, bool allocLock = true, bool clearWhenReturn = true)
    {
    ...
    public MemoryPollAllocatedResult Alloc(ulong allocSize, bool clear = true, bool @lock = true)
    {
        ...
        byte* bytes = pool->Alloc(clear && clearWhenReturn, @lock && allocLock);
    ...
    public bool Return(void* address, ulong blockSize, bool clear = false)
    {
        ...
        bool result = pool != null && pool->Free(address, blockSize, clear && clearWhenReturn);
