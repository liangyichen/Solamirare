...
//                 防止长输入累积雪崩失效
//
//  输出：MemoryFingerprint128（128 位），保留四路状态中的两组独立 ulong，
//        不做最终 XOR 合并，将碰撞概率从 1/2^64 降至 1/2^128。
// ============================================================


/// <summary>
/// 128 位内存指纹，由两个独立的 64 位状态组成。
/// <para>Low 和 High 各自独立派生自不同的累积路径，不可互相推导。</para>
/// </summary>
public readonly struct MemoryFingerprint128
{
    /// <summary>低 64 位（由 h0、h1 路径派生）。</summary>
    public readonly ulong Low;

    /// <summary>高 64 位（由 h2、h3 路径派生）。</summary>
    public readonly ulong High;

    [MethodImpl(MethodImplOptions.AggressiveInlining)]
    internal MemoryFingerprint128(ulong low, ulong high)
    {
        Low  = low;
        High = high;
    }

    /// <summary>表示空/无效状态的零指纹。</summary>
    public static readonly MemoryFingerprint128 Zero;

    public bool Equals(MemoryFingerprint128 other) => Low == other.Low && High == other.High;
    public override int GetHashCode() => HashCode.Combine(Low, High);
    public static bool operator ==(MemoryFingerprint128 a, MemoryFingerprint128 b) => a.Low == b.Low && a.High == b.High;
    public static bool operator !=(MemoryFingerprint128 a, MemoryFingerprint128 b) => a.Low != b.Low || a.High != b.High;
...
/// </summary>
[SkipLocalsInit]
public static unsafe class MemoryFingerprint
{
    ...
    private static readonly delegate*<byte*, uint, MemoryFingerprint128> s_implB;
    private static readonly delegate*<byte*, uint, MemoryFingerprint128> s_implC;

    // ── 静态构造函数：一次性硬件检测 ──
    static MemoryFingerprint()
    {
    ...
    /// <summary>
    /// 计算任意非托管内存段的 128 位指纹。
    /// <para>Low 与 High 由两条完全独立的累积路径派生，碰撞概率约为 1/2^128。</para>
    /// </summary>
    /// <param name="obj">指向内存段起始位置的指针。</param>
    /// <param name="tSize">手动指定内存段字节长度；为 0 时取 sizeof(T)。</param>
    /// <returns>128 位指纹值；输入为 null 或长度为 0 时返回 <see cref="MemoryFingerprint128.Zero"/>。</returns>
    [MethodImpl(MethodImplOptions.AggressiveInlining)]
    public static MemoryFingerprint128 Fingerprint<T>(T* obj, uint tSize = 0)
        where T : unmanaged
    {
        uint TSize = tSize == 0 ? (uint)sizeof(T) : tSize;

        if (obj is null || TSize == 0) return MemoryFingerprint128.Zero;
    ...
    /// <summary>
    /// 四路状态的 finalize，返回 128 位指纹。
    /// <para>Low  = MixStrong(h0 ^ h1 ^ length)  — 由前两路派生</para>
    /// <para>High = MixStrong(h2 ^ h3 ^ ~length) — 由后两路派生，种子取反确保两路初始状态不同</para>
    /// 两路使用不同的 length 扰动，即使 h0==h2、h1==h3 时也会产生不同结果。
    /// </summary>
    [MethodImpl(MethodImplOptions.AggressiveInlining)]
    private static MemoryFingerprint128 FinalMix4(ulong h0, ulong h1, ulong h2, ulong h3, uint length)
    {
        ulong low  = MixStrong(h0 ^ h1 ^  (ulong)length);
        ulong high = MixStrong(h2 ^ h3 ^ ~(ulong)length);
        return new MemoryFingerprint128(low, high);
    }
    ...
    //  适用：length < 128
    // ============================================================
    private static MemoryFingerprint128 FingerprintA_Scalar(byte* ptr, uint length)
    {
    ...
    //    · 尾部回落标量
    // ============================================================
    private static MemoryFingerprint128 FingerprintB_Avx2(byte* ptr, uint length)
    {
    ...
    // ============================================================
    private static MemoryFingerprint128 FingerprintB_AdvSimd(byte* ptr, uint length)
    {
    ...
    // ============================================================
    private static MemoryFingerprint128 FingerprintC_Avx2(byte* ptr, uint length)
    {
        var acc0 = Vector256.Create(C0, C1, C2, C3);
        var acc1 = Vector256.Create(C4, C5, C6, C7);
    ...
    // ============================================================
    private static MemoryFingerprint128 FingerprintC_AdvSimd(byte* ptr, uint length)
    {
        var acc0 = Vector128.Create(C0, C1);
        var acc1 = Vector128.Create(C2, C3);
        var acc2 = Vector128.Create(C4, C5);
