...
public static unsafe class RandomStringGenerator
{
    ...
    static RandomStringGenerator()
    {
        ...
        char* alloc = stackalloc char[] {
            ...
            'a', 'b', 'c', 'd', 'e', 'f', 'g', 'h', 'i', 'j', 'k', 'l', 'm', 'n', 'o', 'p', 'q', 'r', 's', 't', 'u', 'v', 'w', 'x', 'y', 'z',
            // 特殊符号
            '_' //只要这个特殊符号，使得默认的随机字符串绝对不会干扰到json、xml、shell、html、url
        };
        ...
        NativeMemory.Copy(alloc, CharacterPool, byteCount);
    }
    ...
    /// <summary>
    /// <para>根据自定义字符集生成随机字符串，并将结果写入到指定的内存地址。</para>
    /// <para>调用者必须保证指针和长度的有效性，以及自定义字符集的有效性。</para>
    /// </summary>
    /// <param name="destination">指向目标内存块起始位置的指针。</param>
    /// <param name="destinationLength">目标内存块的长度（以字符为单位）。</param>
    /// <param name="customCharset">指向自定义字符集的 UnManagedString 指针，定义了可用的字符范围。</param>
    public static unsafe bool Generate(char* destination, uint destinationLength, UnManagedString* customCharset)
    {
        if (destination == null || destinationLength <= 0 || customCharset == null || customCharset->UsageSize <= 0)
        {
            return false;
        }

        RandomNumberGenerator rng = new RandomNumberGenerator();
        char* charsetPtr = customCharset->Pointer;
        uint charsetLength = customCharset->UsageSize;
        ...
        for (nint i = 0; i < destinationLength; i++)
        {
            ...
            // 直接通过指针将选中的字符写入目标内存
            *(destination + i) = *(charsetPtr + randomIndex);
        }
    ...
    /// <summary>
    /// <para>根据自定义字符集生成随机字符串。</para>
    /// <para>外部调用者注意释放 UnManagedMemory</para>
    /// </summary>
    /// <param name="length">要生成的随机字符串长度。</param>
    /// <param name="customCharset">指向自定义字符集的 UnManagedString 指针，定义了可用的字符范围。</param>
    /// <returns>生成的随机字符串，如果失败则返回空的 UnManagedString。</returns>
    public static UnManagedString Generate(uint length, UnManagedString* customCharset)
    {
        if (length <= 0 || customCharset == null || customCharset->UsageSize <= 0) goto FAILURE;

        UnManagedString memory = new UnManagedString(length, length);

        bool success = Generate(memory.Pointer, memory.UsageSize, customCharset);
