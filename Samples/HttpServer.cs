...
public static unsafe class TestHttpServer
{
    ...
    public static void Start(ushort port)
    {
        ...
        MemoryPoolCluster* _memoryPool = stackalloc MemoryPoolCluster[1];
        memoryPool = _memoryPool;


        memoryPool->Init(schemas, false, false);
    ...
    static bool ResponseOut(UHttpContext* context)
    {
        ...
        ValueDictionary<UnManagedMemory<char>, UnManagedMemory<char>> dic = HttpSeerverResources.JsonString.JsonObjectStringToDictionary(memoryPool);

        UnManagedMemory<char> json = dic.SerializeToJson(memoryPool); 
